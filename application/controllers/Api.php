...
class Api extends CI_Controller
{
    ...
    public function VideoUpload(){  
      $user_id       = $this->input->get_post('user_id');
      $video1        = $this->input->get_post('video1');
      $video2        = $this->input->get_post('video2');
      $video3        = $this->input->get_post('video3');      
      $wheredata=array(
        'user_id'  => $user_id       
      ); 
      ...
      if(!empty($_FILES['video1']['name']))
      {
        ...
        $configVideo['upload_path'] = 'images/videos/';
        $configVideo['allowed_types'] = 'mp4';
        // $configVideo['size'] = 0;
        $configVideo['file_name'] = $video1;
        ...
        if (!$this->upload->do_upload('video1')) {
         $error = array('error' => $this->upload->display_errors());
          ...
          $video1 =  $DetailArray['file_name'];
        }
      }
      if($video1){
        $updateData['video1'] = $video1;
      }else{
      $updateData['video1'] = '';
      }
     ...
     if(!empty($_FILES['video3']['name']))
      {
        ...
        $configVideo['upload_path'] = 'images/videos/';
        $configVideo['allowed_types'] = 'mp4';
        // $configVideo['max_size'] = "2048000";//2mb
        $configVideo['file_name'] = $video3;
        $configVideo['overwrite'] = TRUE;
        $configVideo['remove_spaces'] = TRUE;
    ...
    //get video by user_id
    public function get_Video() {
      $wheredata=array(
          'user_id'=>$this->input->post('user_id')

      ); 
      $result=$this->Main_modal->singleRowdata($wheredata,'user');
      if($result->video1!='' || $result->video2!='' || $result->video3!=''){
        ...
        if($result){
           ...
           $data_result['msg']    = 'Show All Video!'; 
        }
        else{

             $data_result['result'] = 'false';
             $data_result['msg']    = 'Sorry no Video!'; 
        }
     }else{
       $data_result['result'] = 'false';
       $data_result['msg']    = 'Sorry no Video!'; 
      ...
      if($result=$this->Main_modal->selectAllById('user',$wheredata)){
        ...
        if ($result) {
         ...
         // $res_update            = $this->Main_modal->selectAllById('user', $wheredata);
          ...
          $data_result['msg']    = 'User Profile Edit Successfully!';
            
        } else {
          $data_result['result'] = 'false';
          $data_result['msg']    = 'User Profile Not Edit!';
    ...
    public function userLikeUnlike(){
      ...
      $resp_msg_alreay = "";
        ...
        // if($status==4)
        // {
        //   $msg='Visited Your Profile. Please Like this Profile';
        ...
        if(empty($checklike))
        {
            ...
            // {
              ...
              if($status!=2)
              {
                ...
                //    if(!empty($userD)){
                        if(!empty($toUserD)){
                            $this->title = 'You got a notification from Slay Room';
                            $this->message = '';
                            if($status == 1){
    ...
    //count Notification List at user
    ...
    public function Countries_List(){
      $result=$this->Main_modal->select('countries');
      if ($result){
        $data_result['result']='true';
        $data_result['data']= $result;
        $data_result['msg']='all country list';
      }else{
        $data_result['result']='false';
        $data_result['msg']='country not found!!';
      }
       echo json_encode($data_result);
    }

    //all state list by country id
    public function statesBy_countryId(){
      $countryId=$this->input->post('countryId');
      $wheredata=array(
       'country_id'=>$countryId
      );
      $result=$this->Main_modal->selectAllById('states',$wheredata);
      if($result){
        $data_result['result']='true';
        $data_result['msg']='All cities List';
        $data_result['data'] =$result;
        ...
        $data_result['result']='false';
        $data_result['msg']='City Not in List';
      }
      echo json_encode($data_result);
    }

    //all city list by state id
    public function citiesBy_stateId(){
      $stateId=$this->input->post('stateId');
      $wheredata=array(
       'state_id'=>$stateId
      );
      $result=$this->Main_modal->selectAllById('cities',$wheredata);
      if($result){
        $data_result['result']='true';
        $data_result['data']=$result;
        $data_result['msg']='city list';
      }else{
        $data_result['result']='false';
      }
      echo json_encode($data_result);
