        ...
        chat_popup: function () {
        	  var arr = []; // List of users 
				...
				$(document).on('click', '.sidebar-user-box', function() {
					var userImg = $(this).data("image");
					var sender_user = $(this).data("user_id");
					var reciever_user = $(this).data("id");
					var username = $(this).data("name");

					if ($.inArray(reciever_user, arr) != -1) {
						arr.splice($.inArray(reciever_user, arr), 1);
					};
					...
					// send_message.limitToLast(10).on('child_added', function (snapshot) {
				 //    	var data = snapshot.val();
				 //    	var abcd = $('<div class="msg-right">'+data.message+'</div>');
				 //    	$(".msg_body").append(abcd);
					// });

					// recieve_message.limitToLast(10).on('child_added', function (snapshot) {
				 //    	var data = snapshot.val();
				 //    	var abcd = $('<div class="msg-left">'+data.message+'</div>');
				 //    	$(".msg_body").append(abcd);
					// });


					arr.unshift(reciever_user);
					var chatPopup =  '<div class="msg_box" style="right:270px" rel="'+ reciever_user+'" data-id="'+ sender_user+'">'+
					...
					'<div class="msg_footer text-block"><input type="text" placeholder="Type and hit enter" class="msg_input form-control" data-user1="'+ sender_user+'" data-user2="'+ reciever_user+'" ><span class="send-msg-btn"><i class="fa fa-paper-plane-o"></i></span></div></div></div>';     

					$("body").append(chatPopup);
					var chatbox = $('input.msg_input').parents().parents().parents().attr("rel") ;
					//var send_message = firebase.database().ref('messages/'+sender_user);
					firebase.database().ref('/messages/'+sender_user+'/'+reciever_user).on('child_added',function(snapshot)
			        {
			          var data= snapshot.val();
			          ...
			          var senderUid = data.senderUid;
			            ...
			            if(senderUid == sender_user)
			            {
			                 if(type=='text')
			                 {
			                    //''$('<div class="msg-right">'+message+'</div>').appendTo($('.msg_body'));
			                    $('<div class="msg-right">'+message+'</div>').insertBefore('[rel="'+chatbox+'"] .msg_push');
			                 }
			             }else{
			               if(type=='text'){
			               		//$('<div class="msg-left">'+message+'</div>').appendTo($('.msg_body'));
			               		$('<div class="msg-left">'+message+'</div>').insertBefore('[rel="'+chatbox+'"] .msg_push');
			                   }
			             }       
			         });
					displayChatBox();
				 ...
				 $(document).on('click', '.send-msg-btn' , function(e) {
					   ...
					   var user1 = $(this).parent().find('.msg_input').data('user1');
					   var user2 = $(this).parent().find('.msg_input').data('user2');
				...
				function writeUserData(user1, user2, message) {
					...
					var ref_message = firebase.database().ref('messages/');
